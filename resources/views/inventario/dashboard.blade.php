                        ...
                        <tbody>
                            @foreach ($materiales as $m)
                                <tr class="odd:bg-white even:bg-gray-50 hover:bg-gray-100">
                                    ...
                                    <td class="p-2 text-sm text-gray-700 truncate max-w-xs"
                                        title="{{ $m->descripcion }}">{{ $m->descripcion }}</td>
                                    ...
                                    <td class="p-2 text-center space-x-1">
                                        ...
                                        <form action="{{ route('inventario.destroy', $m) }}" method="POST"
                                            class="inline">
                                            @csrf @method('DELETE')
                                            <button type="submit"
                            ...
                            @foreach ($movimientos as $mov)
                                <tr class="odd:bg-white even:bg-gray-50 hover:bg-gray-100">
                                    ...
                                    <td class="p-2 text-sm text-center">
                                        @if ($mov->tipo === 'entrada')
                                            <span
                                                class="px-2 py-1 text-xs rounded-full bg-green-100 text-green-800">Entrada</span>
                                        @else
                                            <span
                                                class="px-2 py-1 text-xs rounded-full bg-red-100 text-red-800">Salida</span>
                                        @endif
                                    </td>
    ...
    <script>
        ...
        document.querySelectorAll('[data-target]').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                document.querySelectorAll('[data-target]').forEach(b => b.classList.remove('border-b-2',
